...
func show_solution(a = null):
	if is_instance_valid($"../../SolutionCheat"):
		...
		if a == 0:
			...
			s.show()
	if is_instance_valid(get_parent().get_node("MazeMap").get_node("MazeMapSolution")):
		var s = get_parent().get_node("MazeMap").get_node("MazeMapSolution")
		if a == 0:
			s.hide()
		elif a == 1:
			s.show()

func teleport_player(a = null):
	if a != null && a is Vector3 && is_instance_valid(get_parent()):
		if get_parent().name == "Player":
			get_parent().global_position = a
